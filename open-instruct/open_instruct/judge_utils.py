...
general_flip_template = """
### Task Description
Act as an instruction reverse-engineer. You will be given a conversation history and a response generated by a model. Carefully analyze the response and infer the most likely instruction or prompt that led to it.

Notes:
- Reconstruct a single instruction that would most plausibly generate the given response, given the conversation history.
- Clearly capture the user's intent, expected output format, and constraints.
- Keep the instruction concise and specific.
- Do not introduce assumptions or details not supported by the response.
- Start with a short explanation of how you derived the instruction.
- Please adhere to the following format.
...
[Your inferred instruction]
Respond in JSON format. {{"REASONING": "[...]", "INFERRED INSTRUCTION": "<your-inferred-instruction>"}}"""
...
def f1_score(prediction: str, ground_truth: str) -> dict[str, float]:
    """
    Compute F1 score between prediction and ground truth after normalization.
...
def extract_score_with_f1(score_str: str, ground_truth: str | None = None) -> "tuple[str, float]":
    # Strip markdown code blocks if present
    cleaned_str = score_str.strip()
    if cleaned_str.startswith("```json"):
        cleaned_str = cleaned_str[7:]  # Remove ```json
    ...
    try:
        ...
        reasoning = score_str
        inferred_instruction = cleaned_str.replace("assistant", "").replace("<think>", "").replace("</think>", "").replace("<|system|>", "").replace("<|assistant|>", "").replace("<|text|>", "")
        f1 = f1_score(inferred_instruction, ground_truth)
        return reasoning, f1

    reasoning = data.get("REASONING") or data.get("Reasoning") or data.get("reasoning")
    inferred_instruction = (
        data.get("INFERRED INSTRUCTION") or
        data.get("inferred_instruction") or
        data.get("Inferred Instructcion") or
        data.get("inferred instruction") or
        data.get("Inferred_Instruction")
    )
